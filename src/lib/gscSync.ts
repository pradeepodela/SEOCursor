 ...
 *
 * It lives here rather than in the API route because the route is no longer
 * the only caller: the MCP server exposes the same sync to an agent, and the
 * stdio transport runs with no HTTP server in front of it. One implementation
 * means the workspace and the agent can never disagree about what a sync did.
 */

import { db } from './db';
import { accessTokenFor, searchAnalytics, daysAgo } from './google';

export type SyncResult = {
  property: string;
  window: { startDate: string; endDate: string; days: number };
  pagesInGsc: number;
  pagesMatchedToCrawl: number;
  keywords: number;
  clicks: number;
  impressions: number;
};

/** Raised when the workspace is not in a state where a sync can run. */
export class GscNotReady extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = 'GscNotReady';
  }
}

export async function syncSearchConsole(siteId: string, days: number): Promise<SyncResult> {
  const conn = await db.gscConnection.findUnique({ where: { siteId } });
  if (!conn) throw new GscNotReady('Search Console is not connected for this website');
  if (!conn.propertyUrl) throw new GscNotReady('No Search Console property is selected');
